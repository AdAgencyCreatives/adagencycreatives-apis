        ...
        function populateTable(creatives) {
            ...
            $.each(creatives, function(index, creative) {
                ...
                var row = '<tr class="sortable-item" id="' + creative.id2 + '">' +
                    ...
                    '<td class="editable-field" data-id="' + creative.id2 + '" data-column="link">' + creative
                    .name +
                    '</td>' +
                    '<td class="sort-order" data-id="' + creative.id2 + '">' + creative.sort_order + '</td>' +
        ...
        $(document).ready(function() {
            ...
            $('table').on('dblclick', '.sort-order', function() {
                ...
                inputField.on('blur', function() {
                    ...
                    $.ajax({
                        url: '/update-featured-creative-order',
                        method: 'POST',
                        data: {
                            creative_id: id,
                            sort_order: newText
...
@section('content')
    ...
    @include('pages.featured_creatives.filters')
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="creative_count_homepage"
                                        value="{{ settings('creative_count_homepage') }}" min="1"
                                        max="{{ \App\Models\Creative::where('is_featured', 1)->count() }}">
                                </div>
                            </div>
                        </div>
